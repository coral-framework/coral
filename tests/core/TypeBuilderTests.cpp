...
TEST(TypeBuilderTests, cyclicDependencies) {
  co::RefPtr<co::ITypeBuilder> structABuilder =
      TestHelper::createBuilder(co::TK_STRUCT, "ciclicDepTest.structs.A");

  // type struct A depends on type struct B, which must be loaded if not loaded
  // yet
  // check if B exists : B should not exist yet
  EXPECT_TRUE(TestHelper::type("ciclicDepTest.structs.B") == NULL);

  // create B
  co::RefPtr<co::ITypeBuilder> structBBuilder =
      TestHelper::createBuilder(co::TK_STRUCT, "ciclicDepTest.structs.B");

  // B struct depends on type A, creating a dependency cycle
  // check if A exists: although type A hasn't finished being created, it must
  // be
  // available from the moment its builder is created, so its type should exist
  // by now
...
TEST(TypeBuilderTests, typeNameInvalidIdentifier) {
  EXPECT_THROW(co::getSystem()->getTypes()->getRootNS()->defineType(
                   "A.Dot.Separated.TypeName", co::TK_STRUCT),
               co::IllegalNameException);
}
...
TEST(TypeBuilderTests, componentInvalidDefinitions) {
  ...
  // create builder
  co::RefPtr<co::ITypeBuilder> cbuilder = TestHelper::createBuilder(
      co::TK_COMPONENT, "TypeBuilderTests_aComponent");

  // a dummy interface to be used in the tests
  co::RefPtr<co::ITypeBuilder> auxbuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "TypeBuilderTests_componentInterface");
  auxbuilder->defineField("test", anyType, true);

  co::IInterface* testInterface =
      co::cast<co::IInterface>(auxbuilder->createType());

  // invalid component class definition
  EXPECT_THROW(cbuilder->defineIdentifier("identifier"),
               co::NotSupportedException);
  EXPECT_THROW(cbuilder->defineField("fieldName", anyType, false),
               co::NotSupportedException);
  EXPECT_THROW(cbuilder->defineBaseType(testInterface),
               co::NotSupportedException);
  EXPECT_THROW(cbuilder->definePort("testIntMember", NULL, false),
...
TEST(TypeBuilderTests, componentDefinition) {
  co::RefPtr<co::ITypeBuilder> componentBuilder = TestHelper::createBuilder(
      co::TK_COMPONENT, "TypeBuilderTests.IComponent");
  co::RefPtr<co::ITypeBuilder> interfaceBuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "TypeBuilderTests.TestInterfaceType");

  interfaceBuilder->defineField("testName", TestHelper::type("string"), true);

  co::IInterface* interfaceTest =
      co::cast<co::IInterface>(interfaceBuilder->createType());

  componentBuilder->definePort("Receptacle", interfaceTest, false);
  componentBuilder->definePort("Facet", interfaceTest, true);

  EXPECT_THROW(componentBuilder->definePort("NullInterface", NULL, false),
               co::IllegalArgumentException);
  EXPECT_NO_THROW(co::getSystem()->getTypes()->getTransaction()->commit());

  co::IComponent* component =
      co::cast<co::IComponent>(componentBuilder->createType());
...
TEST(TypeBuilderTests, componentMissingInput) {
  co::RefPtr<co::ITypeBuilder> componentBuilder = TestHelper::createBuilder(
      co::TK_COMPONENT, "TypeBuilderTests.InvalidComponentType");

  EXPECT_THROW(co::getSystem()->getTypes()->getTransaction()->commit(),
               co::MissingInputException);
  EXPECT_NO_THROW(co::getSystem()->getTypes()->getTransaction()->rollback());

  EXPECT_TRUE(TestHelper::type("TypeBuilderTests.InvalidComponentType") ==
              NULL);
}

TEST(TypeBuilderTests, componentGetInterfaces) {
  co::RefPtr<co::ITypeBuilder> componentBuilder = TestHelper::createBuilder(
      co::TK_COMPONENT, "ComponentTests.secondComponentType");
  co::RefPtr<co::ITypeBuilder> interfaceBuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "ComponentTests.secondTestInterfaceType");
...
TEST(TypeBuilderTests, enumInvalidDefinition) {
  co::RefPtr<co::ITypeBuilder> ebuilder =
      TestHelper::createBuilder(co::TK_ENUM, "TypeBuilderTests_anEnum");
  ...
  // a dummy interface to be used in the tests
  co::RefPtr<co::ITypeBuilder> auxbuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "TypeBuilderTests_enumSuperInterface");
  auxbuilder->defineField("test", anyType, true);

  co::IInterface* testInterface =
      co::cast<co::IInterface>(auxbuilder->createType());

  EXPECT_THROW(ebuilder->defineIdentifier("3identifier"),
               co::IllegalNameException);
  EXPECT_THROW(ebuilder->defineField("fieldName", anyType, false),
               co::NotSupportedException);
  EXPECT_THROW(ebuilder->defineBaseType(testInterface),
               co::NotSupportedException);
...
TEST(TypeBuilderTests, enumDefinition) {
  co::RefPtr<co::ITypeBuilder> builder =
      TestHelper::createBuilder(co::TK_ENUM, "EnumTests.BuilderTestEnum");
...
TEST(TypeBuilderTests, enumMissingInput) {
  co::RefPtr<co::ITypeBuilder> builder =
      TestHelper::createBuilder(co::TK_ENUM, "EnumTests.MissingInputEnum");
...
TEST(TypeBuilderTests, enumCheckIdentifiers) {
  co::RefPtr<co::ITypeBuilder> builder = TestHelper::createBuilder(
      co::TK_ENUM, "EnumTests.CheckIdentifiersTestEnum");
...
TEST(TypeBuilderTests, enumInsertDuplicateIdentifier) {
  co::RefPtr<co::ITypeBuilder> builder = TestHelper::createBuilder(
      co::TK_ENUM, "EnumTests.InsertDuplicateIdentifierTestEnum");
...
TEST(TypeBuilderTests, enumCheckValidIdentifiers) {
  co::RefPtr<co::ITypeBuilder> builder = TestHelper::createBuilder(
      co::TK_ENUM, "EnumTests.checkValidIdentifiersEnum");
...
TEST(TypeBuilderTests, exceptionCreation) {
  ...
  co::RefPtr<co::ITypeBuilder> builder =
      rootNS->defineType("aTypeName", co::TK_EXCEPTION);
...
TEST(TypeBuilderTests, exceptionsAfterCreation) {
  ...
  co::RefPtr<co::ITypeBuilder> builder =
      rootNS->defineType("exceptionsAfterCreationTestType", co::TK_STRUCT);
  ...
  EXPECT_THROW(
      builder->defineField("fullName", TestHelper::type("string"), false),
      co::NotSupportedException);
...
TEST(TypeBuilderTests, interfaceInvalidDefinition) {
  co::RefPtr<co::ITypeBuilder> ibuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "TypeBuilderTests_anInvalidInterface");
  ...
  // a dummy interface to be used in the tests
  co::RefPtr<co::ITypeBuilder> auxbuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "TypeBuilderTests_superInterface");
  auxbuilder->defineField("test", anyType, true);
  ...
  EXPECT_THROW(ibuilder->defineIdentifier("identifier"),
               co::NotSupportedException);
  EXPECT_THROW(ibuilder->defineField("2fieldName", anyType, false),
               co::IllegalNameException);
  EXPECT_THROW(ibuilder->defineBaseType(NULL), co::IllegalArgumentException);
  EXPECT_THROW(ibuilder->definePort("testIntMember", testInterface, false),
               co::NotSupportedException);
...
TEST(TypeBuilderTests, interfaceDefinition) {
  co::RefPtr<co::ITypeBuilder> builder =
      TestHelper::createBuilder(co::TK_INTERFACE, "builderTest.AInterfaceType");
  ...
  co::RefPtr<co::ITypeBuilder> superBuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "builderTest.ASuperInterfaceType");
  ...
  // get the super-type before even finishing creating it (it should be
  // available)
  co::IInterface* super = co::cast<co::IInterface>(
      TestHelper::type("builderTest.ASuperInterfaceType"));
  EXPECT_TRUE(super != NULL);

  // try inheriting from an incompatible type
  EXPECT_THROW(builder->defineBaseType(stringType),
               co::IllegalArgumentException);
  ...
  EXPECT_THROW(builder->defineBaseType(builder->getInterface()),
               co::NotSupportedException);
...
TEST(TypeBuilderTests, interfaceGetMethods) {
  co::RefPtr<co::ITypeBuilder> builder = TestHelper::createBuilder(
      co::TK_INTERFACE, "interfaceGetMethodsTest.Foo");
...
TEST(TypeBuilderTests, nativeClassInvalidDefinitions) {
  ...
  // create builder
  co::RefPtr<co::ITypeBuilder> nbuilder = TestHelper::createBuilder(
      co::TK_NATIVECLASS, "TypeBuilderTests_anNativeClass");

  // a dummy interface to be used in the tests
  co::RefPtr<co::ITypeBuilder> auxbuilder = TestHelper::createBuilder(
      co::TK_INTERFACE, "TypeBuilderTests_anInterface");
  auxbuilder->defineField("test", anyType, true);
  ...
  EXPECT_THROW(nbuilder->defineIdentifier("identifier"),
               co::NotSupportedException);
  EXPECT_THROW(nbuilder->defineField("0_fieldName", anyType, false),
               co::IllegalNameException);
  EXPECT_THROW(nbuilder->defineBaseType(testInterface),
               co::NotSupportedException);
  EXPECT_THROW(nbuilder->definePort("testIntMember", testInterface, false),
               co::NotSupportedException);
...
TEST(TypeBuilderTests, nativeClassDefinition) {
  co::RefPtr<co::ITypeBuilder> builder = TestHelper::createBuilder(
      co::TK_NATIVECLASS, "NativeClassbuilderTest.BuilderTestNativeClass");

  builder->defineField("testName", TestHelper::type("string"), true);
  builder->defineField("age", TestHelper::type("int32"), false);

  co::RefPtr<co::IMethodBuilder> mb = builder->defineMethod("childMethod");
  mb->createMethod();

  co::INativeClass* nativeType =
      co::cast<co::INativeClass>(builder->createType());
...
TEST(TypeBuilderTests, nativeClassMemberClash) {
  co::RefPtr<co::ITypeBuilder> builder = TestHelper::createBuilder(
      co::TK_NATIVECLASS, "NativeClassbuilderTest.MemberClashTestNativeClass");
...
TEST(TypeBuilderTests, structInvalidDefinition) {
  co::RefPtr<co::ITypeBuilder> sbuilder =
      TestHelper::createBuilder(co::TK_STRUCT, "TypeBuilderTests_anStruct");
  ...
  EXPECT_THROW(sbuilder->defineIdentifier("identifier"),
               co::NotSupportedException);
  EXPECT_THROW(sbuilder->defineField("1fieldName", anyType, false),
               co::IllegalNameException);
  EXPECT_THROW(sbuilder->defineBaseType(testInterface),
               co::NotSupportedException);
  EXPECT_THROW(sbuilder->definePort("testIntMember", testInterface, false),
               co::NotSupportedException);
...
TEST(TypeBuilderTests, structMissingInput) {
  co::RefPtr<co::ITypeBuilder> builder =
      TestHelper::createBuilder(co::TK_STRUCT, "aStructType");
...
TEST(TypeBuilderTests, structDefinition) {
  co::RefPtr<co::ITypeBuilder> builder =
      TestHelper::createBuilder(co::TK_STRUCT, "StructTests.BuidlerTestStruct");

  co::IType* int32Type = TestHelper::type("int32");

  builder->defineField("myField", int32Type, false);

  EXPECT_THROW(builder->defineField("myNullTypeField", NULL, false),
               co::IllegalArgumentException);
  EXPECT_THROW(builder->defineField("myReadOnlyField", int32Type, true),
               co::IllegalArgumentException);
  ASSERT_NO_THROW(co::getSystem()->getTypes()->getTransaction()->commit());

  co::IStruct* structType =
      co::cast<co::IStruct>(TestHelper::type("StructTests.BuidlerTestStruct"));
  ...
  ASSERT_TRUE(attr->getType() == int32Type);
