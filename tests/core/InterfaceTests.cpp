...
TEST(InterfaceTests, fieldAndMethodClashes) {
  CSL_TEST_BEGIN(
      "InterfaceTests.FieldAndMethodClashes.methodAndFieldMemberName")
...
TEST(InterfaceTests, invalidDeclarations) {
  ...
  CSL_EXPECT_ERROR("must start with a lowercase letter", "invalidFieldName.csl",
                   7)
  CSL_TEST_END()

  CSL_TEST_BEGIN("InterfaceTests.InvalidDeclarations.missingParameterIO")
  CSL_EXPECT_ERROR("expected 'in', 'out' or 'inout' before 'string'",
                   "missingParameterIO.csl", 5)
  CSL_TEST_END()

  CSL_TEST_BEGIN("InterfaceTests.InvalidDeclarations.parameterTypeNotFound")
  CSL_EXPECT_ERROR("error loading dependency 'IDontExixst'",
                   "parameterTypeNotFound.csl", 6)
  CSL_EXPECT_ERROR("type 'IDontExixst' was not found in the path",
                   "parameterTypeNotFound.csl", 6)
  CSL_TEST_END()

  CSL_TEST_BEGIN("InterfaceTests.InvalidDeclarations.parameterNameClash")
  CSL_EXPECT_ERROR("parameter 'fooMe' defined twice", "parameterNameClash.csl",
                   6)
  CSL_TEST_END()
  ...
  CSL_EXPECT_ERROR("error loading dependency 'IDontExist'",
                   "inexistentReturnType.csl", 4)
  CSL_EXPECT_ERROR("type 'IDontExist' was not found in the pat",
                   "inexistentReturnType.csl", 4)
  CSL_TEST_END()

  CSL_TEST_BEGIN("InterfaceTests.InvalidDeclarations.inexistentRaisedException")
  CSL_EXPECT_ERROR("type 'IDontExist' was not found in the path",
                   "inexistentRaisedException.csl", 5)
  CSL_TEST_END()

  CSL_TEST_BEGIN(
      "InterfaceTests.InvalidDeclarations.missingReturnTypeAndMethodNameClash")
  CSL_EXPECT_ERROR("syntax error near '('",
                   "missingReturnTypeAndMethodNameClash.csl", 4)
  CSL_TEST_END()

  CSL_TEST_BEGIN("InterfaceTests.InvalidDeclarations.RaisesInvalidType")
  CSL_EXPECT_ERROR("attempt to raise non-exception type",
                   "RaisesInvalidType.csl", 5)
  CSL_TEST_END()
}
...
TEST(InterfaceTests, invalidParameter) {
  CSL_TEST_BEGIN(
      "InterfaceTests.InvalidDeclarations."
      "invalidInterfaceWithComponentAsParameter")
  CSL_EXPECT_ERROR("illegal parameter 'comp' of non-data type 'component'",
                   "invalidInterfaceWithComponentAsParameter.csl", 6)
  CSL_TEST_END()

  CSL_TEST_BEGIN(
      "InterfaceTests.InvalidDeclarations."
      "invalidInterfaceWithExceptionAsParameter")
  CSL_EXPECT_ERROR("illegal parameter 'ex' of non-data type 'exception'",
                   "invalidInterfaceWithExceptionAsParameter.csl", 6)
  CSL_TEST_END()
}
