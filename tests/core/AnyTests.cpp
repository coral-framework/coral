...
TEST(AnyTests, stdVectorMemoryLayout) {
  /*
    ...
    This test verifies whether the assumptions are true for the local STL
    distro.

    Basically, the assumption is that the std::vector computes its size() by
    subtracting
    two pointers (one to the start, another past the end of the array). This is
    a fairly
    standard implementation choice for std::vectors, and allows Coral to find
    out how much
    memory is allocated by a std::vector in run-time without knowing its
    template type.

    A derived assumption is that all std::vectors have the same size.
   */

  size_t s1 = sizeof(std::vector<co::int8>);
  ...
  std::vector<co::int8>& bytesVector =
      *reinterpret_cast<std::vector<co::int8>*>(&dblVector);
...
template <typename T>
void constructorValueTest(co::TypeKind kind, const T& sampleValue,
...
template <typename T>
void constructorPtrTest(const co::TypeKind kind, T* sampleValue,
                        const char* expectedTypeName) {
  T* p = sampleValue;
  co::Any a3(p);
...
template <typename T>
void constructorNonConstRefTest(const co::TypeKind kind, const T& sampleValue,
                                const char* expectedTypeName) {
  T v = sampleValue;
...
template <typename T>
void constructorRefTest(const co::TypeKind kind, const T& sampleValue,
                        const char* expectedTypeName) {
...
TEST(AnyTests, constructStruct) {
  ...
  EXPECT_ANY_STREQ(cslError,
                   "out co.CSLError: {filename = \"filename\", line = 7, "
                   "message = \"msg\"}");
}
...
TEST(AnyTests, constructInterface) {
  constructorPtrTest<co::INamespace>(co::TK_INTERFACE,
                                     co::getSystem()->getTypes()->getRootNS(),
                                     "co.INamespace");
}
...
template <typename T> void setAndGetTest(T correctValue, T wrongValue) {
...
TEST(AnyTests, setGetInt64) {
  setAndGetTest<co::int64>(INT64_MIN, INT64_MIN + 1);
  setAndGetTest<co::int64>(INT64_MAX, INT64_MAX - 1);
}
...
TEST(AnyTests, setGetArrays) {
  // --- Slice ---
  co::TSlice<co::IType*> range =
      co::getSystem()->getTypes()->getRootNS()->getTypes();
  co::Any a1(range);
  ...
  std::vector<co::ITypeRef>& refVectorRef =
      a2.get<std::vector<co::ITypeRef>&>();
...
TEST(AnyTests, setGetInterface) {
  ...
  EXPECT_THROW(any.get<co::int32>(), co::IllegalCastException);
...
TEST(AnyTests, equals) {
  ...
  double dbl(3);
  co::Any aDbl(dbl), anInt(3);
  EXPECT_FALSE(aDbl.equals(anInt));
  EXPECT_TRUE(aDbl.equals(3.0));
...
TEST(AnyTests, coercionsFromInt32) {
  ...
  a0.setIn<co::int32>(0);
  co::Any a1;
  a1.setIn<co::int32>(INT16_MAX);
  co::Any a2;
  a2.setIn<co::int32>(INT32_MAX);

  EXPECT_ANY_STREQ(a0, "in int32: 0");
  EXPECT_ANY_STREQ(a1, "in int32: 32767");
  EXPECT_ANY_STREQ(a2, "in int32: 2147483647");
  ...
  EXPECT_EQ(static_cast<double>(INT32_MAX), a2.get<double>());

  // to enum (co::TypeKind)
  EXPECT_EQ(co::TK_NULL, a0.get<co::TypeKind>());
  EXPECT_EQ(co::TK_EXCEPTION,
            co::Any(static_cast<int>(co::TK_EXCEPTION)).get<co::TypeKind>());
...
TEST(AnyTests, coercionsFromEnum) {
  ...
  // to int16
  EXPECT_EQ(static_cast<int>(co::TK_NULL), a0.get<co::int16>());
  EXPECT_EQ(static_cast<int>(co::TK_DOUBLE), a1.get<co::int16>());
  EXPECT_EQ(static_cast<int>(co::TK_COMPONENT), a2.get<co::int16>());

  // to double
  EXPECT_EQ(static_cast<double>(co::TK_NULL), a0.get<double>());
  EXPECT_EQ(static_cast<double>(co::TK_DOUBLE), a1.get<double>());
  EXPECT_EQ(static_cast<double>(co::TK_COMPONENT), a2.get<double>());
...
TEST(AnyTests, coercionsFromSlice) {
  // ValueType
  co::Slice<double> valueRange;
  co::Any valueRangeAny(valueRange);
  EXPECT_ANY_TYPE_STREQ(valueRangeAny, "in double[]");
  ...
  EXPECT_THROW(valueRangeAny.get<co::Slice<int>>(), co::IllegalCastException);
  ...
  EXPECT_THROW(superInterfaceRangeAny.get<co::Slice<co::IInterface*>>(),
...
TEST(AnyTests, coercionsFromStdVector) {
  ...
  EXPECT_THROW(subVecAny.get<std::vector<co::IType*>&>(),
               co::IllegalCastException);
  ...
  EXPECT_THROW(superVecAny.get<co::Slice<co::IInterface*>>(),
               co::IllegalCastException);
}

TEST(AnyTests, putService) {
  co::ITypeRef typeRef;
  co::Any aTypeRef(typeRef);
...
TEST(AnyTests, setVariable) {
  ...
  co::TypeKind anEnum = co::TK_DOUBLE;
...
TEST(AnyTests, setArray) {
  ...
  co::Slice<double> doubleArrayRange;
  ...
  automatic.set(doubleArrayRange);
  manual.set(true, co::getType("double[]"), NULL);
  EXPECT_EQ(automatic, manual);
...
TEST(AnyTests, accessRangeOfDoubles) {
  ...
  for (co::int8 i = 0; i < CORAL_ARRAY_LENGTH(dblArray); ++i) {
    co::Any elem = array[i];
