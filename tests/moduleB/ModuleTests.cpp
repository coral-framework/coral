...
TEST(ModuleTests, setupSystemThenLoadModuleA) {
  ...
  // load moduleA: first call should load, second call should just retrieve the
  // module
...
TEST(ModuleTests, systemAndModuleLifeCycles) {
  ...
  // setup() with moduleA
  system->setup(std::vector<std::string>{"moduleA"});
  ...
  EXPECT_EQ(co::ModuleState::Integrated, moduleA->getState());

  // moduleB should not have been loaded yet
  ASSERT_TRUE(system->getModules()->findModule("moduleB") == NULL);

  // loading moduleB now should bring it directly to the
  // ModuleState_PresentationIntegrated state
  moduleB = system->getModules()->load("moduleB");
  ASSERT_TRUE(moduleB != NULL);
  EXPECT_EQ(co::ModuleState::Integrated, moduleB->getState());

  // ok to tearDown() now
  system->tearDown();

  // all modules should now be in the ModuleState_Disposed state
  EXPECT_EQ(co::ModuleState::Disposed, moduleA->getState());
  EXPECT_EQ(co::ModuleState::Disposed, moduleB->getState());

  // cannot load additional modules after the system is torn down
  EXPECT_THROW(system->getModules()->load("whatever"),
               co::IllegalStateException);
...
TEST(ModuleTests, crossModuleInheritance) {
  // instantiate a component from 'moduleA' that implements an interface from
  // 'co'
...
TEST(ModuleTests, crossModuleReflection) {
  ...
  try {
    ...
    EXPECT_EQ("illegal instance (moduleA.TestStruct expected, got double)",
              e.getMessage());
  }
...
TEST(ModuleTests, serviceDependencies) {
  ...
  // re-register the service provided by moduleA, forcing its immediate
  // instantiation
  sm->setIsLazy(false);
...
TEST(ModuleTests, luaScriptedNoArg) {
  try {
    ...
    EXPECT_EQ(
        "error obtaining a reflector for type 'moduleB.LuaScriptedNoArg'"
        " via provider '@lua.Scripted':"
        " this annotation requires a script name as argument",
        e.getMessage());
  }
}
...
TEST(ModuleTests, luaScriptedMissing) {
  try {
    ...
    EXPECT_EQ(
        0, e.getMessage().find(
               "error obtaining a reflector for"
...
TEST(ModuleTests, luaScriptedError) {
  try {
    ...
    EXPECT_EQ(0,
              e.getMessage().find(
                  "error obtaining a reflector for type"
                  " 'moduleB.LuaScriptedError' via provider '@lua.Scripted':"));
    EXPECT_NE(std::string::npos,
              e.getMessage().find("faulty.lua:1: syntax error near 'script'"));
  }
}
...
TEST(ModuleTests, luaScriptedEmpty) {
  try {
    ...
    EXPECT_EQ(
        "error obtaining a reflector for type 'moduleB.LuaScriptedEmpty'"
        " via provider '@lua.Scripted': script 'moduleB.empty' does"
        " not implement 'moduleB.LuaScriptedEmpty'",
        e.getMessage());
  }
}
