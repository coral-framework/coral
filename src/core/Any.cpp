...
inline void cast(const void* from, void* to) {
  *reinterpret_cast<To*>(to) =
      static_cast<To>(*reinterpret_cast<const From*>(from));
}
...
inline void castTo(TypeKind fromKind, const void* from, void* to) {
  switch (fromKind) {
    case TK_BOOL:
      cast<bool, To>(from, to);
      break;
    case TK_INT8:
      cast<int8, To>(from, to);
      break;
    case TK_INT16:
      cast<int16, To>(from, to);
      break;
    case TK_INT32:
      cast<int32, To>(from, to);
      break;
    case TK_INT64:
      cast<int64, To>(from, to);
      break;
    case TK_DOUBLE:
      cast<double, To>(from, to);
      break;
    case TK_ENUM:
      cast<int16, To>(from, to);
      break;
...
inline void castFrom(const void* from, TypeKind toKind, void* to) {
  switch (toKind) {
    case TK_BOOL:
      cast<From, bool>(from, to);
      break;
    case TK_INT8:
      cast<From, int8>(from, to);
      break;
    case TK_INT16:
      cast<From, int16>(from, to);
      break;
    case TK_INT32:
      cast<From, int32>(from, to);
      break;
    case TK_INT64:
      cast<From, int64>(from, to);
      break;
    case TK_DOUBLE:
      cast<From, double>(from, to);
      break;
    case TK_ENUM:
      cast<From, int16>(from, to);
      break;
...
void castCountable(TypeKind fromKind, const void* from, TypeKind toKind,
                   void* to) {
  assert(isCountable(toKind) && isCountable(fromKind));
  switch (fromKind) {
    case TK_BOOL:
      castFrom<bool>(from, toKind, to);
      break;
    case TK_INT8:
      castFrom<int8>(from, toKind, to);
      break;
    case TK_INT16:
      castFrom<int16>(from, toKind, to);
      break;
    case TK_INT32:
      castFrom<int32>(from, toKind, to);
      break;
    case TK_INT64:
      castFrom<int64>(from, toKind, to);
      break;
    case TK_DOUBLE:
      castFrom<double>(from, toKind, to);
      break;
    case TK_ENUM:
      castFrom<int16>(from, toKind, to);
      break;
...
template <typename T> inline bool isEqual(const void* p1, const void* p2) {
  return *reinterpret_cast<const T*>(p1) == *reinterpret_cast<const T*>(p2);
}

template <typename T> inline void copy(const void* from, void* to) {
  *reinterpret_cast<T*>(to) = *reinterpret_cast<T const*>(from);
}

bool isTrue(const std::string& str) { return str == "true"; }
...
void streamOut(std::ostream& os, const co::Any& var) {
  ...
  switch (kind) {
    ...
    case TK_DOUBLE: {
      double scalar;
      castTo<double>(kind, var.isIn() ? &var.state.data : var.state.data.ptr,
                     &scalar);
    ...
    case TK_ENUM: {
      int16 id = var.state.data.i16;
      TSlice<std::string> ids =
          static_cast<IEnum*>(var.getType())->getIdentifiers();
      if (id >= 0 && id < static_cast<int16>(ids.getSize()))
        os << ids[id];
      else
        os << "<INVALID ID #" << id << ">";
    } break;
...
typedef std::vector<int8> StdVector;

inline int8* getVectorData(void* ptr) {
...
inline int8* getVectorData(const __any::State& s) {
...
#define THROW_NO_CONVERSION(from, to, MORE_MESSAGES)              \
  CORAL_THROW(IllegalCastException, "no conversion from '"        \
                                        << from << "' to '" << to \
                                        << "'" MORE_MESSAGES)
...
bool Any::equals(const Any& other) const {
  ...
  if (kind == TK_ARRAY) {
    ...
    if (count != b.state.size) return false;

    IType* elemType = static_cast<IArray*>(a.getType())->getElementType();
    return elemType->getReflector()->compareValues(a.state.data.ptr,
                                                   b.state.data.ptr, count);
  }

  __any::Data* pA(&a.state.data), *pB(&b.state.data);
  if (!isCountable(kind) && !isReference(kind)) {
    pA = pA->deref;
    pB = pB->deref;
  }
...
void Any::set(bool isIn, IType* type, const void* addr, size_t size) {
  ...
  switch (k) {
    case TK_BOOL:
      copy<bool>(addr, &state.data);
      break;
    case TK_INT8:
      copy<int8>(addr, &state.data);
      break;
    case TK_INT16:
      copy<int16>(addr, &state.data);
      break;
    case TK_INT32:
      copy<int32>(addr, &state.data);
      break;
    case TK_INT64:
      copy<int64>(addr, &state.data);
      break;
    case TK_DOUBLE:
      copy<double>(addr, &state.data);
      break;
    case TK_ENUM:
      copy<int16>(addr, &state.data);
      break;
    case TK_INTERFACE:
      copy<IService*>(addr, &state.data);
      break;
    default:
      assert(!isCountable(k));
  }
}
...
void Any::put(Any in) const {
  ...
  if (myK == TK_NULL || !isOut())
    CORAL_THROW(IllegalStateException, "cannot write to a '" << state
                                                             << "' variable");
  ...
  switch (myK) {
    ...
    case TK_INT64:
      if (isCountable(inK))
        castCountable(inK, &in.state.data, myK, state.data.ptr);
      else if (inK == TK_STRING) {
        const std::string& str = *in.state.data.str;
        int64 v = strtol(str.c_str(), NULL, 0);
        castFrom<int64>(&v, myK, state.data.ptr);
      } else
        THROW_NO_CONVERSION(in.state, state, );
      break;
    ...
    case TK_DOUBLE:
      if (isCountable(inK))
        ...
        state.data.deref->d = strtod(in.state.data.str->c_str(), NULL);
      else
        THROW_NO_CONVERSION(in.state, state, );
      break;
    ...
    case TK_ARRAY:
      ...
      if (in.getKind() != TK_ARRAY) THROW_NO_CONVERSION(in.state, state, );
      ...
      if (getType() == in.getType()) {
        ...
        elemType->getReflector()->copyValues(
            in.state.data.ptr, getVectorData(state.data.ptr), in.state.size);
      } else {
        // slow path - each element must be converted through put()
        for (size_t i = 0; i < in.state.size; ++i) at(i).put(in[i]);
      }
      break;

    case TK_STRUCT:
    case TK_NATIVECLASS:
      if (in.getType() != getType()) THROW_NO_CONVERSION(in.state, state, );
      getType()->getReflector()->copyValues(in.state.data.ptr, state.data.ptr,
                                            1);
      break;
...
void Any::resize(size_t n) const {
  ...
  if (currentN == n) return;  // nothing to be done
...
bool Any::operator==(const Any& other) const {
  ...
  if (state.isIn != other.state.isIn) return false;

  TypeKind k = getKind();
  if (state.isIn) {
    if (k == TK_ARRAY && state.size != other.state.size) return false;

    switch (k) {
      case TK_BOOL:
        return isEqual<bool>(&state.data, &other.state.data);
      case TK_INT8:
        return isEqual<int8>(&state.data, &other.state.data);
      case TK_INT16:
        return isEqual<int16>(&state.data, &other.state.data);
      case TK_INT32:
        return isEqual<int32>(&state.data, &other.state.data);
      case TK_INT64:
        return isEqual<int64>(&state.data, &other.state.data);
      case TK_DOUBLE:
        return isEqual<double>(&state.data, &other.state.data);
      case TK_ENUM:
        return isEqual<int16>(&state.data, &other.state.data);
      default:
        assert(!isCountable(k));
...
void Any::cast(Any& to) const {
    ...
    } else  // casting from an 'in' to an 'in' variable
    {
      if (toK == TK_INTERFACE) {
        ...
        if (toK == TK_ARRAY) {
          ...
          if (myET == toET ||
              (myET->getKind() == TK_INTERFACE && myET->isA(toET))) {
  ...
  CORAL_THROW(IllegalCastException, "illegal cast from '" << state << "' to '"
                                                          << to.state << "'");
}
...
void AnyValue::convert(IType* type) {
  ...
  co::AnyValue temp;
  temp.create(type);
  if (isValid()) temp.getAny().put(getAny());
  swap(temp);
}
...
std::ostream& operator<<(std::ostream& os, const co::__any::State& s) {
  assert(s.type);
  if (s.type->getKind() != co::TK_NULL) os << (s.isIn ? "in " : "out ");
