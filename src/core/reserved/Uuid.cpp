...
inline void generateRandomBytes(int8* data) {
  ...
  if (CryptAcquireContext(&cryptProvider, NULL, NULL, PROV_RSA_FULL,
    ...
    if (CryptGenRandom(cryptProvider, 16, data))
      if (CryptReleaseContext(cryptProvider, 0)) return;
  assert(false);
  for (unsigned int i = 0; i < 16; ++i) data[i] = 0;
#elif defined(CORAL_OS_UNIX)
  FILE* fd = fopen("/dev/urandom", "r");
  for (unsigned int i = 0; i < 16; ++i) data[i] = static_cast<int8>(fgetc(fd));
  fclose(fd);
#else
...
Uuid::Uuid(const std::string& text) { setString(text); }

bool Uuid::isNull() const {
  for (int i = 0; i < 4; i++)
    if (_data.dwords[i] != 0) return false;
  return true;
}

void Uuid::set(int8 bytes[]) { std::copy(bytes, bytes + 16, _data.bytes); }
...
void Uuid::getString(std::string& str) const {
  ...
  for (int i = 0; i < 16; ++i) {
    std::uint8_t v = _data.bytes[i];
    str.push_back(HEX_CHARS[v / 16]);
    str.push_back(HEX_CHARS[v % 16]);
    if (i == 3 || i == 5 || i == 7) str.push_back('-');
  }
}
...
inline std::uint8_t hex2nibble(int8 c) {
...
inline int8 hex2byte(int8 c1, int8 c2) {
...
void Uuid::setString(const std::string& str) {
  ...
  static const int8 NUM_CHARS = 35;

  if ((last - first + 1) == NUM_CHARS) {
    int8 i = 0, bytesRead = 0;
    while (true) {
      // positions where we expect a '-'
      if (i == 8 || i == 13 || i == 18) {
        if (first[i] != '-') break;  // malformed string
        ++i;
        continue;
      }

      int8 hexChar1 = first[i++];
      int8 hexChar2 = first[i++];

      if (!isHex(hexChar1) || !isHex(hexChar2)) break;
...
void Uuid::clear() { std::fill(_data.dwords, _data.dwords + 4, 0); }
