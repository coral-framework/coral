...
bool co::OS::isFile(const std::string& path) {
  ...
  if (CORAL_STAT_FUNC(path.c_str(), &info)) return false;
...
void co::OS::convertDotsToDirSeps(std::string& dotSeparatedPath) {
  ...
  for (size_t i = 0; i < len; ++i) {
    if (dotSeparatedPath[i] == '.') dotSeparatedPath[i] = CORAL_OS_DIR_SEP;
  }
}
...
void co::OS::normalizePath(std::string& path) {
  ...
  while (st.getNext(token)) {
    ...
    if (len == 0 || (len == 1 && token[0] == '.')) continue;
...
bool co::OS::searchFile2(co::Slice<std::string> listA,
                         co::Slice<std::string> listB, std::string& path,
                         std::string* partA, std::string* partB) {
  std::string filePath;
