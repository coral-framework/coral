...
class CryptoHashAlgorithm {
 public:
  ...
  virtual void addData(const char* data, size_t length) = 0;
  ...
  inline void assertNoResult() {
    if (_hasResult) {
      CORAL_THROW(IllegalStateException,
                  "The hash result has already been calculated. "
...
class Sha1CryptoHashAlgorithm : public CryptoHashAlgorithm {
 public:
  Sha1CryptoHashAlgorithm() { reset(); }

  virtual ~Sha1CryptoHashAlgorithm() {
    // empty;
  }

  virtual void addData(const char* data, size_t length) {
    assertNoResult();
  ...
  virtual void getResult(CryptoHash::Result& res) const {
    if (!_hasResult) {
      sha1::final(&_ctx, reinterpret_cast<std::uint8_t*>(_result));
      _hasResult = true;
    }
 ...
 private:
  ...
  mutable int8 _result[sha1::DIGEST_SIZE];
};
...
CryptoHash::~CryptoHash() { delete _algorithm; }
...
void CryptoHash::Result::toUuid(Uuid& uuid) {
  uuid.set(reinterpret_cast<int8*>(data.bytes));
}

void CryptoHash::addData(const char* data, size_t length) {
  _algorithm->addData(data, length);
}

void CryptoHash::getResult(Result& res) const { _algorithm->getResult(res); }

void CryptoHash::reset() { _algorithm->reset(); }

CryptoHash::CryptoHash(const CryptoHash& other) { *this = other; }
