#ifndef _CO_PRIVATE_OS_H_
#define _CO_PRIVATE_OS_H_
...
class CORAL_EXPORT OS {
 public:
  ...
  /*!
    ...
    On UNIX, that means it begins with a slash; on Windows that it begins with a
    drive letter.
   */
  ...
  /*!
    ...
    This method changes the string inplace, replacing every dot with a directory
    separator.
   */
  ...
  /*!
    \brief Searches for a file in a space defined by a combination of two string
    lists.

    File path combinations are evaluated in the same order they're listed, and
    in case of
    ambiguities the search always returns the first match.

    \param[in] listA List of base directory paths (e.g. pass the CORAL_PATH
    here).
    \param[in] listB List of relative file paths (e.g. potential file names).
    \param[out] path Path to the first file found (only set if the method
    returns true).
    \param[out] partA optional; set with the substring of \a path that came from
    \a listA.
    \param[out] partB optional; set with the substring of \a path that came from
    \a listB.

    \return true if a file is found; false otherwise.
  */
  static bool searchFile2(co::Slice<std::string> listA,
                          co::Slice<std::string> listB, std::string& path,
                          std::string* partA = 0, std::string* partB = 0);

  /*!
    \brief Searches for a file in a space defined by a combination of three
    string lists.
    ...
    \param[in] listA Outer-most list of base directory paths (e.g. pass the
    CORAL_PATH here).
    \param[in] listB Central list of relative directory paths (e.g. pass a list
    of module names here).
    \param[in] listC Inner-most list of relative file paths (e.g. pass a list of
    file names here).
    \param[out] path Path to the first file found (only set if the method
    returns true).
    \param[out] partA optional; set with the substring of \a path that came from
    ...
    \param[out] partB optional; set with the substring of \a path that came from
    \a listB.
    \param[out] partC optional; set with the substring of \a path that came from
    \a listC.
  ...
  */
  static bool searchFile3(co::Slice<std::string> listA,
                          ...
                          co::Slice<std::string> listC, std::string& path,
                          std::string* partA = 0, std::string* partB = 0,
                          std::string* partC = 0);
};
