...
void LuaState::setup() {
  ...
  // pre-load the LPeg library
  lua_pushcfunction(sm_L, luaopen_lpeg);
  lua_setfield(sm_L, -2, "lpeg");
...
bool LuaState::findScript(lua_State*, const std::string& name,
                          std::string& filename) {
...
void LuaState::push(lua_State* L, co::Any var) {
  ...
  switch (k) {
    ...
    case co::TK_BOOL:
      ...
      break;
    case co::TK_INT8:
      lua_pushinteger(L, s.data.i8);
      break;
    case co::TK_INT16:
      lua_pushinteger(L, s.data.i16);
      break;
    case co::TK_INT32:
...
void LuaState::get(lua_State* L, int index, const co::Any& var) {
  ...
  switch (lua_type(L, index)) {
    case LUA_TNIL:
      ...
      break;
    case LUA_TBOOLEAN:
      var.put(lua_toboolean(L, index) != 0);
      break;
    ...
    case LUA_TTABLE: {
      ...
      if (tp == LUA_TNIL) {
        ...
        if (k == co::TK_ARRAY) {
          ...
          if (k != co::TK_ANY)
            CORAL_THROW(lua::Exception, "no conversion from Lua array to '"
                                            << var.state << "'");
        ...
        lua_pop(L, 1);
        if (unexpectedType)
          CORAL_THROW(lua::Exception, "Coral object expected, got "
                                          << unexpectedType);
      }
    } break;
...
co::int32 LuaState::call(const std::string& moduleName,
  ...
  try {
    ...
    // should we retrieve a function from (presumably) the returned module
    // table?
    if (!functionName.empty()) {
      ...
      if (lua_isnil(L, -1)) throw lua::Exception("no such function");
    }

    // push the args and call the function
    int numArgs = static_cast<int>(args.getSize());
    for (int i = 0; i < numArgs; ++i) push(L, args[i]);
    ...
    if (!functionName.empty()) ss << "'" << functionName << "' ";
...
void LuaState::collectGarbage() { lua_gc(getL(), LUA_GCCOLLECT, 0); }
...
void LuaState::getArray(lua_State* L, int index, const co::Any& array) {
  ...
  try {
    ...
    lua_settop(L, stackTop);
    CORAL_THROW(lua::Exception, "invalid array element #" << i << " ("
                                                          << e.what() << ")");
  }
}
  ...
  if (size >= 5 && msg[0] == '{') {
    ...
    if (msgStart) {
      ...
      try {
        ...
        CORAL_THROW(lua::Exception, "[error throwing an exception of type '"
                                        << typeName << "' from Lua: "
                                        << e.what() << "] " << parsedMsg);
      }
