-------------------------------------------------------------------------------
-- Coral CSL Compiler Module v1 (based on Coral)
-------------------------------------------------------------------------------
...
local Compiler = {
  ...
  simulation = false,         -- if true disables code generation
...
function Compiler:generateJSON()
  assert( not self.updatedTypes and #self.types == 0, "this compiler instance has already been used" )
  self:loadModuleTypes()
  self.log( "Exporting module '" .. self.moduleName .. "' to JSON..." )
  expand( self.outDir, self.moduleName .. '.json', json, self )
  self.log( "All data has been written to " .. self.outDir .. '/' .. self.moduleName .. ".json" )
end
...
function Compiler:generateModule()
  self:loadModuleTypes()

  self.moduleNS = self.utils.toCppName( self.moduleName )
